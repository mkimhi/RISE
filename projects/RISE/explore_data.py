...
from detectron2.projects.idol import add_rise_config, build_detection_train_loader, build_detection_test_loader
...
def create_sparse_ds(args):
    cfg = setup(args)

    loader = Trainer.build_train_loader(cfg)
    loader_iterator = iter(loader.dataset)
    data = next(loader_iterator)
    for i,data in enumerate(loader.dataset):
        cv2.imwrite(f'./image_{i}_0.png',data["image"][0].permute (1,2,0).numpy().astype('uint8'))
        cv2.imwrite(f'./image_{i}_1.png',data["image"][1].permute (1,2,0).numpy().astype('uint8'))
