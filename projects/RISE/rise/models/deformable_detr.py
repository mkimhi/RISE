...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs,  targets, ref_target, indices, num_boxes, log=True):
        ...
        for batch_idx in range(batch_size):
            ...
            target_classes_o_mul_idx = target_classes_o[gt_multi_idx]
            if len(target_classes_o_mul_idx.shape) == 2:
                target_classes_o_mul_idx = target_classes_o_mul_idx.squeeze()
            target_classes[batch_idx,valid_query] = target_classes_o_mul_idx # target_classes_o[gt_multi_idx]
