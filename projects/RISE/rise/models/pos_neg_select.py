...
def get_pos_idx(bz_boxes,bz_out_prob,bz_gtboxs,bz_tgt_ids,valid):
    with torch.no_grad():  
        if False in valid:
            ...
            #    valid[0] = True #to delete!!!!!!!!!!!!!!!!!!!!
            #    bz_gtboxs = bz_boxes[bz_tgt_ids[valid]]
            #    bz_tgt_ids = bz_tgt_ids[valid]
           
            #if valid.shape[0] < bz_gtboxs.shape[0]: 
            #    valid = valid.repeat(bz_gtboxs.shape[0])
            #if torch.sum(valid) == 0:    
            #    valid[0] = True
        ...
        pred_box = box_cxcywh_to_xyxy(bz_boxes)
        tgt_box = box_cxcywh_to_xyxy(bz_gtboxs)
        if (tgt_box[:, 2:] >= tgt_box[:, :2]).all():
            cost_giou = -generalized_box_iou(pred_box, tgt_box )
        else:
            cost_giou = torch.zeros(1).to(cost_class.device)
        if len(cost_class.shape)==3:
            cost_class = cost_class.squeeze(1)
        if cost_class.nelement() == 0:
            cost_class = torch.zeros(1).to(cost_class.device)
