...
def load_ytvis_json(json_file, image_root, dataset_name=None, extra_annotation_keys=None,old_load=False):
...
"""
    ...
    for (vid_dict, anno_dict_list) in vids_anns:
        ...
        #TODO: fix!!!!!
        if 'unlabeled' not in dataset_name and 'val' not in dataset_name and record["length"]>2 and not old_load:
            continue
        video_objs = []
...
def find_frames_ids_from_json(dicts,n_frames=2):
    changed_frames_d={}
    for i,d in enumerate(dicts):
        vid_name = d["file_names"][0].split('/')[-2]
        vid_id = d['video_id'] #check by id
        ...
        if not found:
            ...
            continue
            changed_frames_d[vid_name]= 5 #None
            changed_frames_d[vid_id] = 5 #None

    return changed_frames_d



def overwrite_json(json_file,changed_frames_dict,new_json_file,fraction):
    ...
    return _json
#01c4cb5ffe - fixed annotation
#00917dcfc4 - first frame?
...
if __name__ == "__main__":
    ...
    new_json_file = "./datasets/ytvis_2021/annotations/instances_train_sub_2f.json"
    if fraction!=1:
        new_json_file = f"./datasets/ytvis_2021/annotations/instances_train_sub_2f_{str(fraction)}.json"
    ...
    changed_frames_dict = find_frames_ids_from_json(dicts)



    _ = overwrite_json(json_file,changed_frames_dict, new_json_file,fraction)    
    #00917dcfc4
