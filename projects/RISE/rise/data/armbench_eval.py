...
class ARMBENCHEvaluator(DatasetEvaluator):
    """
    ...
    def __init__(
        ...
        *,
        ...
        with contextlib.redirect_stdout(io.StringIO()):
            self._coco_api = ArmBench(json_file)
    ...
    def process(self, inputs, outputs):
        ...
        """for input, output in zip(inputs, outputs):
            ...
            if "instances" in output:
                ...
                prediction["instances"] = instances_to_coco_json(instances, input["id"])
    ...
    def _eval_predictions(self, predictions, img_ids=None):
        ...
        coco_results = predictions #list(itertools.chain(*[x["instances"] for x in predictions]))
...
def _evaluate_predictions_on_coco(
    ...
    for res in coco_results:
        res['image_id'] = res['id']
        res['segmentation'] = res['segmentations'] 
    coco_dt = coco_gt.loadRes(coco_results)
    #RISE creates 2 identical annotations
    for ann in coco_dt.anns.values():
        ann['bbox'] = ann['bbox'][0]
        ann['segmentation'] = ann['segmentation'][0]
        ann['area'] = ann['area'][0]
        ann['segmentations'] = ann['segmentations'][0] #can delete..
...
class ARMBENCHEvaluator2(DatasetEvaluator):
    """
    Evaluate AR for object proposals, AP for instance detection/segmentation, AP
    for keypoint detection outputs using COCO's metrics.
    See http://cocodataset.org/#detection-eval and
    ...
    def __init__(
        ...
        *,
        ...
        json_file = PathManager.get_local_path(self._metadata.json_file)
        with contextlib.redirect_stdout(io.StringIO()):
            self._armbench_api = ArmBench(json_file)

        # Test set json files do not contain annotations (evaluation must be
        # performed using the COCO evaluation server).
        self._do_evaluation = "annotations" in self._armbench_api.dataset
    ...
    def evaluate(self):
        ...
        #add_coco_batch(coco_eval, inputs, outputs, targets)
